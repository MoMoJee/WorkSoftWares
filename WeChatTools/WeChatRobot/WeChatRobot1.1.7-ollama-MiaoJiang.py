"""
WeChatRobot1.1.7-ollama-MiaoJiang更新：
更换到了本地AI接口，完善了对多种模型的兼容
新增模型更换指令
修复了在本地端口占用时服务无法启动的问题
"""
...
def chat_with_kimi(user_message, role, retry_count=0 ):
    ...
    try:
        ...
        # 将Kimi的回复添加到对话历史中
        conversation_history.append({"role": "assistant", "content": completion['message']['content']})
        # 返回Kimi的回复
        prompt_tokens=0 # 换成本地模型后怎么确定？我不知道
...
#AI接口接入部分
model = "llama3.2:3b"
logger.info("成功连接.Model：" + str(model))
Comsumption = 0
