...
import sys
# 添加模块路径
sys.path.append('D:\\python_learn\\WorkSoftWares\\WeChatTools\\WeChatRobot\\Console_Command')
import Console_Command1_0
from Console_Command1_0 import Console_Command
...
def save_conversation_history_to_file(conversation_history, folder_path):
    ...
    # 构造文件名，包含日期和时间
    file_name = f'Kimi-MinJiang-conversation_history_{current_time}.txt'
    # 构造完整的文件路径
...
def chat_with_kimi(user_message, role, retry_count=0 ):
    ...
    try:
        ...
        if prompt_tokens > 7373:#创建tokens过载逻辑，占用90%以上时触发
            ...
            return completion.choices[0].message.content + "\n还有就是，不好意思喵，民酱的上下文存储快要满了，不得不忘记一些事情了\n~至于我还记得哪些事情，就看谁给民酱留下的印象最深刻咯~"
        else:
            return completion.choices[0].message.content
...
def handle_error(e, retry_count=0):
    ...
    if error_type == 'content_filter':
        error_message = "民酱不想回答啦~您的输入或生成内容可能包含不安全或敏感内容喵~"
    elif error_type == 'invalid_request_error':
        if "token length too long" in message:
            error_message = "请求中的 tokens 长度过长，民酱记不住啦~请求不要超过模型 tokens 的最长限制呦~。"
            conversation_history = clear_n_percent_of_history(conversation_history)  #调用清理函数，随机删除25%聊天数据
            logger.info("已删除25%历史记录")
            conversation_history.extend([#extend,而不是append
                {"role": "system", "content": "请自然对话，你现在的角色是可爱二次元女孩，名字叫做民酱"},
            ...
            error_message = "呜呜 请求的 tokens 数和设置的 max_tokens 加和超过了模型规格长度~。让民酱休息一下啦~"
            conversation_history = clear_n_percent_of_history(conversation_history)  #调用清理函数，随机删除25%聊天数据
            logger.info("已删除25%历史记录")
            conversation_history.extend([#extend,而不是append
                {"role": "system", "content": "请自然对话，你现在的角色是可爱二次元女孩，名字叫做民酱"},
        ...
        error_message = "民酱饿了，账户额度不足，请检查账户余额。"
        ...
        error_message = "当前并发请求过多喵~节点限流中~别急别急 民酱一个一个回答~"
    elif error_type == 'server_error':
        error_message = "内部错误，请联系管理员。"
    else:
        error_message = f"发生了一个未知错误: {message},民酱也不知道该怎么办了"
...
def generate_cute_exit_message():
    ...
    exit_messages = [
        "呜~主人要离开民酱了，我会想你的~记得下次再来找民酱玩哦~",
        "~是时候说再见了，民酱会在这里等你的~下次见喵~",
        "啊呜~主人要忙了吗？民酱会乖乖的，期待和主人的下次相遇~",
        "~主人要休息了吗？民酱也要好好休息，下次再见要更加精神哦~",
        "~时间过得真快，主人要离开了，民酱会一直在这里，期待你的再次召唤~"
    ]
    # 随机选择一条退出语句
...
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
# 构造文件名，包含时间戳
file_name = f'Kimi-MinJiang-log_{current_time}.txt'
# 构造完整的文件路径
full_file_path = os.path.join(folder_path, file_name)
...
while 1:
    ...
    for chat in msgs:
        if First:
            chat.SendMsg('AI民酱机器人启动！注意以下所有的【民酱】发言都源自AI大模型，不代表本人观点，和中央民族大学无关，请谨慎识别！')
            logger.info('AI民酱机器人启动！注意以下所有的【民酱】发言都源自AI大模型，不代表本人观点，和中央民族大学无关，请谨慎识别！')
            First = 0
        if OutBreak:
            break
             ...
             # ！！！ 回复收到，此处为`chat`而不是`wx` ！！！
                ...
                if "@@退出" in f'{msg.content}':
                    ...
                    if "#sys" in f'{msg.content}':
                        ...
                        print(f"民酱system: {kimi_reply}")
                        logger.info('system请求回复：' + f"民酱system: {kimi_reply}")
                        if f'{kimi_reply}' != "None":  # 这里是 最早的错误返回是一个None，现在虽然不再是了但是还是留着吧
                            chat.SendMsg(f'【民酱system】{kimi_reply}')
                        else:
                            chat.SendMsg('消息发得太快啦，请一会儿再试喔')
                            time.sleep(10)
                            break
                        ...
                        chat.SendMsg("【民酱Console Command】" + Console_Command(f'{msg.content}'))#跳转到控制台指令处理
                        ...
                        if "民酱" in f'{msg.content}':
                            ...
                            user_input = f'{sender.rjust(20)}：{msg.content}'
                            kimi_reply = chat_with_kimi(user_input , "user")
                            print(f"民酱对主人说: {kimi_reply}")
                            if f'{kimi_reply}' != "None":  # 这里是 最早的错误返回是一个None，现在虽然不再是了但是还是留着吧
                                chat.SendMsg(f'【民酱】{kimi_reply}')
                                logger.info('民酱回复主人：' + f'【民酱】{kimi_reply}')
                            else:
                                chat.SendMsg('消息发得太快啦，请一会儿再试喔')
                                time.sleep(10)
                                break
