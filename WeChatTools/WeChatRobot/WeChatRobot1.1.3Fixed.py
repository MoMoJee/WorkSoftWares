...
def chat_with_kimi(user_message, role, retry_count=0 ):
    ...
    try:
        ...
        # 将Kimi的回复添加到对话历史中
        conversation_history.append({"role": "assistant", "content": completion.choices[0].message.content})#这里缺少在输出消息为空时的处理，只能规避
...
logger.info("微信接入成功")
listen_list = ["五号楼花果山"]
# 然后调用`AddListenChat`方法添加,监听对象，其中可选参数`savepic`为是否保存新消息图片
