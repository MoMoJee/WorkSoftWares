"""
1.1.3Fixed-Tongyi更新
更换了大模型，给出了模型切换接口。后期会支持对话中切换模型、自动选择模型等，实现图片处理、图片生成
修复初始化功能
注意：错误处理模块未更新，错误返回可能会比较唐突，见谅
通义怎么蠢蠢的，不喜欢
"""
...
def save_conversation_history_to_file(conversation_history, folder_path):
    ...
    # 构造文件名，包含日期和时间
    file_name = f'TY-conversation_history_{current_time}.txt'
    # 构造完整的文件路径
...
def chat_with_kimi(user_message, role, retry_count=0 ):
    ...
    try:
        ...
        # 返回Kimi的回复
        if prompt_tokens > 7373:#创建tokens过载逻辑，占用90%以上时触发（这是moonshot用的，通义限制要少）
            logger.warning('剩余的上下文长度不足10%，当前占用tokens：' + str(prompt_tokens) + '，' + str(int(prompt_tokens*100/131072)) + "%")
...
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
# 构造文件名，包含时间戳
file_name = f'TY-log_{current_time}.txt'
# 构造完整的文件路径
full_file_path = os.path.join(folder_path, file_name)
...
api_key = "sk-4448f56ccf1644b7acad39f5f48e790b"
base_url = "https://dashscope.aliyuncs.com/compatible-mode/v1"
model = "qwen-plus-2024-07-23"
client = OpenAI(
    api_key=api_key,
    base_url=base_url
)
